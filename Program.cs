...
namespace Library_Management_System
{
    internal class Program
    {
                ...
                switch (choice)
                {
                    ...
                    case 8:
                        ...
                        else
                        {
                            ...
                            int.TryParse(Console.ReadLine(), out int bCopies);
                            Console.Write("Enter genre (press Enter to skip, default = General): ");
                            string bGenre = Console.ReadLine();

                            if (bGenre.Trim() == "")
                                RegisterBook(bTitle, bAuthor, bCopies);
                            else
                                RegisterBook(bTitle, bAuthor, bCopies, bGenre);
                        }
                        break;
        ...
        static void RegisterMember()
        {
            ...
            Console.WriteLine("\nMember registered! Temporary ID: " + memberId);
        ...
        {
            string namePart = memberName.Substring(0, Math.Min(4, memberName.Length)).ToUpper();
            long ticks = DateTime.Now.Ticks;
            int sqrtPart = (int)Math.Sqrt(ticks % 99991);

            return namePart + "-" + sqrtPart.ToString("D4");
        }

        static void DisplayBookDetails(string title, string author, string genre, int copies)
        {
            Console.WriteLine("--- Book Details ---");
            Console.WriteLine("Title:  " + title.PadRight(25));
            Console.WriteLine("Author: " + author.PadRight(25));
            Console.WriteLine("Genre:  " + genre.PadRight(25));
            Console.WriteLine("Copies: " + Convert.ToString(copies).PadRight(25));

        }

        static double CalculateRenewalFee(int days)
        {
            double fee = Math.Ceiling(days * 0.4);
        ...
        static bool UpdateMemberEmail(string newEmail, out string cleanedEmail)
        {
            cleanedEmail = newEmail.Trim();

            if (cleanedEmail.Contains("@") && cleanedEmail.Length >= 6)
                return true;
        ...
        static void PrintSessionSummary()
        {
            ...
            Console.WriteLine("Books Borrowed: " + Convert.ToString(totalBorrowed));
            Console.WriteLine("Total Fines Paid: " + Math.Round(totalFinesPaid, 2).ToString("F2") + " OMR");
            Console.WriteLine("Session Date: " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
        }

        static void PrintMemberStatistics()
        {
            Console.WriteLine("--- Member Statistics ---");
            Console.WriteLine("Name: " + memberName);
            Console.WriteLine("Tier: " + memberTier);
            Console.WriteLine("Books Borrowed: " + totalBorrowed);
        }

        static void PrintMemberStatistics(bool detailed)
        {
            PrintMemberStatistics();
            if (detailed)
            {
                ...
                Console.WriteLine("Expiry Date: " + membershipExpiry);
            }
        }
